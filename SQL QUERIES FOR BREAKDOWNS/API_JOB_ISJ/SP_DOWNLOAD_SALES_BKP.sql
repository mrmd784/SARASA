...
/****** Object:  StoredProcedure [dbo].[SP_DOWNLOAD_SALES_BKP]    Script Date: 11/09/2023 15:56:59 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[SP_DOWNLOAD_SALES_BKP]
@CuLoca	VarChar(3)='01'
AS
SET NOCOUNT ON
DECLARE @lastid 	int
...
Select @lastid=IsNull(Max(LastId),0) From tb_DownloadIdx Where Iid='SAL3' AND LOCA=@CuLoca
Delete From tb_DownloadIdx Where Iid='SAL3' And LastId<@lastid AND LOCA=@CuLoca
...
,RecallOrder,Shift,RefItemCode,Pax,descripplus,CustName,RefRowNo,Rc,TempReceipt,InvoiceNo,ReceiptEncrypt from OpenRowset('Sqloledb','124.43.76.88';'sa';'tstc123',EasyWay2.dbo.tbPosTransact2) WHERE Idx>@lastid Order By Idx
...
BEGIN     	
Insert Into tbPosTransact([ItemCode],[RefCode],[Descrip],[Cost],[Price],[Qty],[Amount],[IDI1],[IDis1],[IDiscount1]
...
Insert Into tb_DownloadIdx ([Iid],[lastid],[LOCA]) Values ('SAL3',@Idx,@CuLoca)
        FETCH NEXT FROM cur_trns
	INTO @Idx,@ItemCode,@RefCode,@Descrip,@Cost,@Price,@Qty,@Amount,@IDI1,@IDis1,@IDiscount1,@IDI2,@IDis2,@IDiscount2
,@IDI3,@IDis3,@IDiscount3,@IDI4,@IDis4,@IDiscount4,@IDI5,@IDis5,@IDiscount5,@Rate,@SDStat,@SDNo,@SDID,@SDIs,@SDiscount
,@Tax,@Nett,@Loca,@IId,@Receipt,@Salesman,@Cust,@Cashier,@StartTime,@EndTime,@RecDate,@BillType,@Unit,@UnitNo,@RowNo,@CDate
...
Select @lastid=IsNull(Max(LastId),0) From tb_DownloadIdx Where Iid='SAL4' AND LOCA=@CuLoca
Delete From tb_DownloadIdx Where Iid='SAL4' And LastId<@lastid  AND LOCA=@CuLoca
...
,TempReceipt,InvoiceNo,ReceiptEncrypt from OpenRowset('Sqloledb','124.43.76.88';'sa';'tstc123',EasyWay2.dbo.tbPosPayment2) WHERE Idx>@lastid Order By Idx
...
WHILE @@FETCH_STATUS = 0
BEGIN     	
     Insert Into tbPosPayment([Receipt],[Loca],[UnitNo],[Sdate],[Id],[TypeId],[Amount],[Balance],[Cashier]
...
,@TempReceipt,@InvoiceNo,@ReceiptEncrypt)
	...
	Insert Into tb_DownloadIdx ([Iid],[lastid],[loca]) Values ('SAL4',@Idx,@CuLoca)
     FETCH NEXT FROM cur_pay
INTO @Idx,@Receipt,@Loca,@UnitNo,@RecDate,@Id,@TypeId,@Amount,@Balance,@Cashier,@PaidAt,@BillType,@SerialNo,@PStatus,@Cust,@CustType
,@BankId,@ChequeDate,@UpdateBy,@AdvanceNo,@Upload,@ZNo,@invoice,@Insertdate,@Did,@PId,@Serial,@PaymentNo,@TypeId2,@InvSerial,@NewCustCode
,@CurId,@CurRate,@CurCode,@CurRateRel,@CurRateLkr,@CurIdDef,@RealAmount,@FLoca,@TableNo,@OrderNo,@Shift,@CustName,@CardRate,@ErpId,@Rc,@PStatusX
