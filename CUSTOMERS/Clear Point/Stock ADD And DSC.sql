...
select *  from tbPostransact where loca='01' and Upload='B'
select Distinct Loca,UnitNo,zno from tbPostransact where loca='01' and Upload='B'

Select UnitNo,loca, Isnull(Sum(Case Iid When '001' Then Nett
 When '002' Then -Nett Else 0 End),0) AS Nett  From tbPosTransact 
Where Status=1 And SaleType='S' And Upload='B' and loca='01' group by UnitNo,Loca
--Net 9546475.336 (Chack net value)
...
--QTY 21879.074 (Chack all qty)

---Change Upload Staus(For Not Update as sale)
select Distinct Upload from tbPostransact where loca='01'

update tbPostransact set Upload='B' where loca='01' and Upload is null and ZNo in('333','334','335','336','337','338','339')
update tbPospayment set Upload='B' where loca='01' and Upload is null and ZNo in('333','334','335','336','337','338','339')
...
From tbPostransact Where Upload='B'  And loca='01' and zno in ('333','334','335','336','337','338','339') and Status = '1' Group By ItemCode
/*
Insert Into tb_DwnStock(getQty,ItemCode,Cost,Price,Nett) 
...
Select * From tb_System Where LocaCode = '01' --229

--
update tb_System set opb=OPB+2 Where LocaCode = '01' --230 & 231

--Check whether transactions happen after stock take
select * from tb_Stock where LocaCode='01' order by pdate desc

select SUM(getqty) from tb_DwnStock    --21879.0740
...
Select
	'01'+'000230','01','DSC' + Convert(Varchar(10),'2023-07-18'),'2023-07-18',Rtrim
...
Insert Into tb_Stock ([SerialNo],[LocaCode],[RefNo],[PDate],[SuppCode],[RepCode],[TourCode],
	[ItemCode],[Scale],[PackSize],[Qty],[Balance],[Rate],[Cost],[RealCost],[Type],[ID],[CrDate],[Status],[IType])
Select
	'01'+'000231','01','ADD' + Convert(Varchar(10),'2023-07-18'),'2023-07-18',Rtrim
	(I.Supp_Code),Null,'StkUp',Rtrim(I.Item_Code),'Each','1'
...
,D.PWhole_Price,'2023-07-18','EasyWay','ITC',GetDate(),1,S.Cost,S.Cost From tb_Item As I Join tb_ItemDet As D
...
Select	 	             '01000230',   '01'   ,'DSCSTK','2023-07-18',I.Supp_Code,D.ItemCode,I.Inv_Descrip,'Each' ,'StockUP' ,E.Cost_Price,E.ERet_Price,abs(D.ADQTY),abs(D.ADQty*E.Cost_Price),abs(D.ADQty*E.ERet_Price),GetDate(),'DSC',   '1'  ,  '1' ,    '1'   ,    ''   ,   ''
...
Select 					  '01000230',   '01'   ,'DSC2023-07-18','2023-07-18',Sum(CostValue),Sum(RetValue),'DSC',  '1' ,   '1'  ,GetDate(),  'Sarasa'
From tb_StAdjDet Where LocaCode = '01' And SerialNo = '01000230'

--ADD

Insert Into tb_StAdjDet	([SerialNo],[LocaCode],[RefNo] ,[IDate]     ,[SuppCode] ,[ItemCode],[ItemDescrip],[Scale],[Remark],[Cost]      ,[Rate]      ,[Qty],[CostValue]         ,[RetValue]          ,[TrDate] ,[ID] ,[Status],[LnNo],[PackSize],[ExpDate],[BatchNo])
Select	 	             '01000231',   '01'   ,'ADDSTK','2023-07-18',I.Supp_Code,D.ItemCode,I.Inv_Descrip,'Each' ,'StockUP' ,E.Cost_Price,E.ERet_Price,(D.ADQTY),(D.ADQty*E.Cost_Price),(D.ADQty*E.ERet_Price),GetDate(),'ADD',   '1'  ,  '1' ,    '1'   ,    ''   ,   ''
From tb_ItemDet As E Join tb_DwnStock As D On E.Item_Code = D.ItemCode Join tb_Item As I On D.ItemCode = I.Item_Code
Where E.Loca_Code = '01' and d.ADQTY>0 

Insert Into tb_StAdjSumm ([SerialNo],[LocaCode],[RefNo]       ,[IDate]     ,[CostValue]   ,[RetValue]   ,[Id] ,[Type],[Status],[TrDate] ,[UserName])			
Select 					  '01000231',   '01'   ,'ADD2023-07-18','2023-07-18',Sum(CostValue),Sum(RetValue),'ADD',  '1' ,   '1'  ,GetDate(),  'Sarasa'
From tb_StAdjDet Where LocaCode = '01' And SerialNo = '01000231'
...
From tb_Stock where status='1' and LocaCode='01' order by pdate desc
...
Insert Into tb_Stock_Backup ([Idx],[SerialNo],[LocaCode],[RefNo],[PDate],[SuppCode],[RepCode],[TourCode],[ItemCode],[Scale],[PackSize],[Qty],[Balance],[Rate],[Cost],[RealCost],[Type],[ID],[CrDate],[Status],[IType],[BkpDate],[CNo])
	Select					 [Idx],[SerialNo],[LocaCode],[RefNo],[PDate],[SuppCode],[RepCode],[TourCode],[ItemCode],[Scale],[PackSize],[Qty],[Balance],[Rate],[Cost],[RealCost],[Type],[ID],[CrDate],[Status],[IType],GetDate(),  0 
From tb_Stock Where LocaCode = '01' And Status = '1' and  PDate<'2023-07-18' order by pdate desc
*/
