...
/****** Object:  StoredProcedure [dbo].[spDiscountUpdateSubTotProm]    Script Date: 06/05/2024 10:56:11 ******/
SET ANSI_NULLS ON
GO
...
Create Table #tbTempPromo (PromoId Int,DiscountP Decimal(12,4),Discount Decimal(12,4),PType Int,Loyalty Int,IsDiscountPercentage bit)
...
If (Not Exists(Select * From #tbTempPromo) And @p_Amount<>0 and @p_Cust='0') --Auto Discount All
Begin
	If (@p_PromId=0 and @p_Cust='0') --If User Not Specify Which Promotion To Apply
	Begin
		...
		And Convert(Varchar(5),Getdate(),108) Between TimeFrom And TimeTo And (BillTotal<=@p_Amount) And PromType In (1,2,3,6,7,8,9,10)
		))or PromId not in(select distinct PromId from tbPromotionMaster)) and PromId<>0
		
		update x
		set Nett=temp.Nett+(--temp.IDiscount1+temp.IDiscount2+temp.IDiscount3+temp.IDiscount4+temp.IDiscount5
		...
		Rate=temp.Rate+(--temp.IDiscount1/temp.Qty)
						--+(temp.IDiscount2/temp.Qty)
						--+(temp.IDiscount3/temp.Qty)
						--+(temp.IDiscount4/temp.Qty)
						--+(temp.IDiscount5/temp.Qty)
		...
		Truncate Table #tbTempPromo
		Insert Into #tbTempPromo (PromoId,DiscountP,PType,Discount,Loyalty,IsDiscountPercentage) Select 
		PromId,PromDiscount,PromType,0,OnlyLoyalty,IsDiscountPercentage from tbPromotionMaster Where Status=1 And 
		Convert(Datetime,Convert(Varchar(11),Getdate(),103),103) Between DateFrom And DateTo
		...
		Begin
			Delete From #tbTempPromo Where PType Not In (6,7,8,9,10,12)
		End
		
		--if Exists(Select 1 from #tbTempPromo where PType=8)
		--Begin
		--	Delete From #tbTempPromo Where PType in (9,10,12)
		--End
		...
		while(exists(select * from #tbTempPromo where PType=10) and exists(select * from #temp10))
		begin
			...
			update x set 
				...
				--IDI3 = (case when not(IDI2=0) then 6 else 0 end),
				--IDI2 = (case when not(IDI1=0) then 6 else 0 end),
				IDI1 = 6,--(case when (IDI1=0) then 6 else IDI1 end),
				--IDis5 = (case when (not(IDI4=0) and not(TotalPromItemsValue=0)) then CONVERT(varchar,cast(b.TotalPromItemsValue as float))+'%' else '' end),
				--IDis4 = (case when (not(IDI3=0) and not(TotalPromItemsValue=0)) then CONVERT(varchar,cast(b.TotalPromItemsValue as float))+'%' else '' end),
				--IDis3 = (case when (not(IDI2=0) and not(TotalPromItemsValue=0)) then CONVERT(varchar,cast(b.TotalPromItemsValue as float))+'%' else '' end),
				--IDis2 = (case when (not(IDI1=0) and not(TotalPromItemsValue=0)) then CONVERT(varchar,cast(b.TotalPromItemsValue as float))+'%' else '' end),
				IDis1 = case when not(TotalPromItemsValue=0) then CONVERT(varchar,cast(b.TotalPromItemsValue as float))+'%' else '' end,--(case when ((IDI1=0) and not(TotalPromItemsValue=0)) then CONVERT(varchar,cast(b.TotalPromItemsValue as float))+'%' else IDis1 end),
				...
				IDiscount1 = (case when not(TotalPromItemsValue=0) then x.Qty*b.TotalPromItemsValue*(x.Price/100) when not(DisAmount=0) then x.Qty*b.DisAmount else 0 end),--(case when (IDI1=0) then (case when not(TotalPromItemsValue=0) then x.Qty*b.TotalPromItemsValue*(x.Rate/100) when not(DisAmount=0) then x.Qty*b.DisAmount else 0 end) else IDiscount1 end),
				Rate=x.Price-(case when (b.TotalPromItemsValue>0) then (b.TotalPromItemsValue*x.Price/100) else b.DisAmount end),
				Nett=(x.Price*x.Qty)-(case when (b.TotalPromItemsValue>0) then (b.TotalPromItemsValue*x.Qty*x.Price/100) else x.Qty*b.DisAmount end),
				PromId=@PromId
			from tbXitem x 
			join tbItemsBuy b on x.ItemCode=b.ItemCode and (x.PromId=0)-- or x.PromId=b.PromId)
			join #tbTempPromo t on b.PromId=t.PromoId
		...
		Begin
			Select Top 1 @PromId=PromoId,@p_Discount=DiscountP,@IsLoyalty=Isnull(Loyalty,'0'),@l_IsPercentage=IsDiscountPercentage from #tbTempPromo Where PType=7 
			Order By Discount Desc
			Set @p_SDId=6
			if @l_IsPercentage=1
			begin
		...
		Begin
			Select Top 1 @PromId=PromoId,@p_Discount=DiscountP,@IsLoyalty=Isnull(Loyalty,'0'),@l_IsPercentage=IsDiscountPercentage from #tbTempPromo Where PType=6 
			Order By Discount Desc
			Set @p_SDId=6
			if @l_IsPercentage=1
			begin
		...
		Begin
			...
			Select Top 1 @PromId=PromoId,@p_Discount=DiscountP,@IsLoyalty=Isnull(Loyalty,'0'),@l_IsPercentage=IsDiscountPercentage from #tbTempPromo Where 
			PType=8 Order By Discount Desc
			
			Select @ItemCount=Isnull(sum(TotalPromItemsQty),0) From tbItemsBuy Where PromId=@PromId
			...
			Begin
				exec spRemoveDisc @p_loca,@p_UnitNo,@p_Receipt,'**'
		...
		Begin
			Select Top 1 @PromId=PromoId,@p_Discount=DiscountP,@IsLoyalty=Isnull(Loyalty,'0'),@l_IsPercentage=IsDiscountPercentage
			from #tbTempPromo Where 
