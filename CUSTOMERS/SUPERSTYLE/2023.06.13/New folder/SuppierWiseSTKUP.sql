...
---------------------------------------------------------------------------------------------------------------------------------------------
--delete  from tbPostransact2 where loca='03' and UnitNo='3' and ZNo='2141' and Upload='A'
--delete  from tbPospayment2 where loca='03' and UnitNo='3' and ZNo='2141' and Upload='A'
--Check DataTransfer Ok Or Not
select *  from tbPostransact2 where loca='03' and UnitNo='3' and ZNo='2141' and Upload='A'
select Distinct Loca,UnitNo,zno from tbPostransact2 where loca='03' and   UnitNo='3' and upload is null

Select UnitNo,LOCA,Isnull(Sum(Case Iid When '001' Then Nett
 When '002' Then -Nett Else 0 End),0) AS Nett  From tbPosTransact2 
Where Status=1 And SaleType='S' And Upload='A' and loca='03'   Group By UnitNo,LOCA
--7809798

Select Isnull(Sum(Case Iid When '001' Then qty
When '002' Then -Qty Else 0 End),0) AS qty  From tbPosTransact2
Where Status=1 And SaleType='S' And Upload='A' and loca='03'  Group By UnitNo 

---Change Upload Staus(For Not Update as sale)
select Distinct Upload from tbPostransact where loca='03'

update tbPostransact2 set Upload='A' where loca='03'  and ZNo in('2141') and Upload is null
update tbPospayment2 set Upload='A' where loca='03' and ZNo in('2141') and Upload is null

--select * from tbPostransact2 where loca='03'  and ZNo in('2141') and Upload is null
...
From tbPostransact2 Where Upload='A'  And loca='03' and  Status = '1' Group By ItemCode,Cost,Price

--2.(Check Item not in easyway)
select * from tbPostransact2 Where Loca = '03' and Upload = 'A' and ItemCode not in(select item_code from tb_Item)

--3.
Update tb_DwnStock Set Nett = (Qty * Price)
Select SUM(Nett) From tb_DwnStock  ---7809798.00

--4.Update Document no for Stock Adjestment
Select * From tb_System Where LocaCode = '03'    --13022
update tb_System set OPB='13023' Where LocaCode = '03'

--5.Check If There Any Transactions After Stock Take

--UPDATE C  SET  C.BatchNo=D.Supp_Code FROM  TB_ITEM  AS  D  JOIN tb_Stock AS C ON D.ITEM_CODE=C.ITEMCODE where locacode='03' and Status='1' and batchno is null

Select * From tb_Stock Where LocaCode = '03' And Status = '1' and batchno='E004' order by pdate desc

--6.Stock Zero
Update tb_Stock Set Status = '9', TourCode = 'STK14/23' Where LocaCode = '03' And Status = '1' and BatchNo='E004'
and PDate<'2023-06-13'

select * from tb_Stock Where LocaCode = '03' And Status = '1' and BatchNo='E004'
and PDate<'2023-06-13' order by PDate desc
...
Select
	'03'+'013023','03','OB' + Convert(Varchar(10),'2023-06-12'),'2023-06-12',Rtrim(I.Supp_Code),Null,'StkUp'
...
,D.PWhole_Price,'2023-06-12','EasyWay','ITC',GetDate(),1,S.Cost,S.Cost From tb_Item As I Join tb_ItemDet As D
...
From tb_Stock Where LocaCode = '03' And Status = '1' and BatchNo='E004' and PDate<'2023-06-13'
...
From tb_Stock Where LocaCode = '03' And Status = '9' and  TourCode = 'STK14/23'
...
Select	 	             '03013023',   '03'   ,'OPBSTK','2023-06-12',I.Supp_Code,D.ItemCode,I.Inv_Descrip,'Each' ,'StockUP' ,E.Cost_Price,E.ERet_Price,D.Qty,(D.Qty*E.Cost_Price),(D.Qty*E.ERet_Price),GetDate(),'OPB',   '1'  ,  '1' ,    '1'   ,    ''   ,   ''
...
Select 					  '03013023',   '03'   ,'OP2023-06-12','2023-06-12',Sum(CostValue),Sum(RetValue),'OPB',  '1' ,   '1'  ,GetDate(),  'Sarasa'
From tb_StAdjDet Where LocaCode = '03' And SerialNo = '03013023'
